...
class EmailSendService extends CoreService
{
    ...
    public function emailBaseAuth(EmailTemplate $emailTemplate, User $user): PHPMailer
    {
        ...
        $emailSetting = data_get($emailTemplate, 'emailSetting', EmailSetting::first());
        ...
        $mail->SMTPAuth     = $emailSetting->smtp_auth;
        $mail->SMTPDebug    = $emailSetting->smtp_debug;*/
        $mail->Host         = data_get($emailSetting, 'host');
        $mail->Port         = data_get($emailSetting, 'port');
        $mail->Username     = data_get($emailSetting, 'from_to');
        $mail->Password     = data_get($emailSetting, 'password');
        $mail->SMTPSecure   = PHPMailer::ENCRYPTION_STARTTLS;
