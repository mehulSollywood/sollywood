...
export default function SellerRefundDetails() {
  ...
  const { user, order: data } = activeMenu?.data || {};
  const refundDetail = activeMenu?.data;
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'product',
      key: 'product',
      render: (_, row) => row.shopProduct?.product?.translation?.title,
    },
    {
      title: t('image'),
      dataIndex: 'img',
      ...
      <Card
        ...
        title={
          <>
            ...
            {t('from.order')} {user?.firstname} {user?.lastname || ''}
          </>
        }
        extra={
        ...
        {data?.status !== 'canceled' && (
          <Col span={24}>
            <Card title={t('order.status')}>
              <Steps
                current={statuses?.findIndex((item) => item === data?.status)}
              >
                {statuses?.slice(0, -1).map((item) => (
                  <Steps.Step key={item.id} title={t(item)} />
                ))}
              </Steps>
            </Card>
          </Col>
        ...
        <Col span={16}>
            ...
            <Card style={{ minHeight: '200px' }}>
              <Row hidden={loading} className='mb-3 order_detail'>
                <Col span={12}>
                  ...
                  <div>
                    {t('cause')}:
                    <span className='ml-2'>{refundDetail?.message_user}</span>
                  </div>
                  <br />
                  <div>
                    {t('answer')}:
                    <span className='ml-2'>
                      {refundDetail?.message_seller || 'not.answered.yet'}
                    </span>
                  </div>
                </Col>
                <Col span={12}>
                  <div>
                    ...
                    <span className='ml-2'>
                      {refundDetail?.status === 'pending' ? (
                        <Tag color='blue'>{t(refundDetail?.status)}</Tag>
                      ) : refundDetail?.status === 'canceled' ? (
                        <Tag color='error'>{t(refundDetail?.status)}</Tag>
                      ) : (
                        <Tag color='cyan'>{t(refundDetail?.status)}</Tag>
                      )}
                      {refundDetail?.status !== 'canceled' && (
                        <EditOutlined
          ...
          <Card className='w-100 order-table'>
            <Table
              ...
              scroll={{ x: 1024 }}
              columns={columns}
              dataSource={refundDetail?.order?.details || []}
              loading={loading}
          ...
          <Card title={<Space>{t('customer.info')}</Space>}>
            <div className='d-flex w-100 customer-info-container'>
              {loading ? (
                ...
                <Avatar shape='square' size={64} src={IMG_URL + user?.img} />
              )}
              ...
              <div className='customer-info-detail'>
                ...
                <div className='customer-info'>
                  ...
                  <span className='description'>
                    <MdEmail />
                    {loading ? <Skeleton.Button size={16} /> : user?.email}
                  </span>
                </div>
                <div className='customer-info'>
